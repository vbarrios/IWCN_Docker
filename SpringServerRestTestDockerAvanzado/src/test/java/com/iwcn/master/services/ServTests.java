...
public class ServTests {
	...
	public void init(){
		...
		allProducts.add(p2);
		when(productoRepository.findAll()).thenReturn(allProducts);
	...
	public void getProductsTest()  throws ServiceException {
		List<Producto>allProducts = dataBaseServices.getAll();
	...
	@Test
	public void constructorProductoTest() throws ServiceException {
		Producto p = new Producto();
		assertEquals(p.getName(), null);
		assertEquals(p.getReference(), 0);
		assertEquals(p.getPrice(), 0);
		assertEquals(p.getStock(), 0);
		imprimir("constructorProduct ok");
